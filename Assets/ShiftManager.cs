...
public class ShiftManager : MonoBehaviour
{
    ...
    private void OnCustomerGaveUpWaiting(Customer customer)
    {
        ...
        StartCoroutine(GameOverAfterCustomerLeavesRoutine(customer, " лиент ушЄл не дождавшись заказа"));
    }
    ...
    private IEnumerator GameOverAfterCustomerLeavesRoutine(Customer leavingCustomer, string reason)
    {
        ...
        // ждЄм, пока этот конкретный клиент реально выйдет (Destroy -> ссылка станет null)
        float timeout = 12f;
        float t = 0f;
