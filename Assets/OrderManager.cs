...
public class OrderManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (resultText != null)
        {
            resultText.text = "ќжидание клиента...";
    ...
    public bool Submit(Plate plate)
    {
        ...
        // 1) —читаем штрафные очки
        var (points, report) = CalculatePenaltyPoints(plate);

        // 2) ќпредел€ем настроение клиента по очкам + учитываем "вечно злого"
        ...
        // === TIME-BASED MOOD (тво€ логика 60/60/40 и т.д.) ===
        ...
        // ЅерЄм худшее настроение из (points mood) и (time mood)
        resultMood = WorstMood(resultMood, timeMood);


        // 3) ќбновл€ем UI (по желанию, удобно дл€ отладки)
        if (resultText != null)
        {
            resultText.text = report;
        }

        // 4) «аказ завершЄн (чтобы таймер не тикал дальше)
        orderActive = false;

        // 5) —“ј–“ реакции: Thinking -> Result -> (после звука) уход
        if (reactionCoroutine != null)
            StopCoroutine(reactionCoroutine);

        reactionCoroutine = StartCoroutine(HandleCustomerReaction(customer, resultMood));

        // 6) ќчистка тарелки/спавн новой Ч это у теб€ уже было раньше.
        // ≈сли у теб€ здесь была логика очистки/удалени€ Ч оставь еЄ как было.

        // Submit прин€т (мы запустили реакцию)
        return true;
    }
    ...
    private (int points, string report) CalculatePenaltyPoints(Plate plate)
    {
        ...
        {
            points += 10;
            report.AppendLine("+10 пуста€ тарелка");
        }

        // === 1)  ќ“Ћ≈“џ: сыра€ / пережаренна€ ===
        ...
        if (plate.DirtyFlags != null)
        {
            for (int i = 0; i < plate.DirtyFlags.Count; i++)
            {
                ...
                {
                    points += 10;
                    report.AppendLine($"+10 гр€зный ингредиент #{i + 1}");
                }
            }
        }
        ...
        {
            points += 5;
            report.AppendLine("+5 слишком долгое приготовление (75Ц99%)");
        }
        else if (ratio >= 0.50f && ratio < 0.75f)
        {
            points += 2;
            report.AppendLine("+2 среднее врем€ приготовлени€ (50Ц74%)");
        }
        ...
        // === »“ќ√ ===
        report.AppendLine($"»“ќ√ќ Ў“–ј‘: {points}");
